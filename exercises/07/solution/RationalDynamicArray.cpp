...
RationalDynamicArray::Iterator::Iterator(RationalDynamicArray& arr, int index)
	: arr(&arr), index(index) {
	Iterator::numberOfInstances++;
}

RationalDynamicArray::Iterator::Iterator()
	: arr(0), index(-1) {
	Iterator::numberOfInstances++;
}

RationalDynamicArray::Iterator::Iterator(const Iterator& other)
	: arr(other.arr), index(other.index) {
	Iterator::numberOfInstances++;
}
...
Rational& RationalDynamicArray::Iterator::operator *() {
	assert(arr != 0);
...
bool RationalDynamicArray::Iterator::operator ==(const Iterator& other) const {
...
RationalDynamicArray::Iterator& RationalDynamicArray::Iterator::operator ++() {
	assert(arr != 0 && index < arr->getSize());

	index++;
	return *this;
}

RationalDynamicArray::Iterator RationalDynamicArray::Iterator::operator ++(int) {
	assert(arr != 0 && index < arr->getSize());

	RationalDynamicArray::Iterator result = *this;
	index++;
...
int RationalDynamicArray::Iterator::numberOfInstances = 0;

int RationalDynamicArray::Iterator::getNumberOfInstances() {
	return Iterator::numberOfInstances;
}

RationalDynamicArray::Iterator RationalDynamicArray::begin() {
	return Iterator(*this, 0);
}

RationalDynamicArray::Iterator RationalDynamicArray::end() {
